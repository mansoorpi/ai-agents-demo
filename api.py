...
def chat_endpoint(req:ChatRequest):
    prior=load_messages(req.session_id)
    results=retrieve(req.message,DATA_DIR)
    ...
    messages=[system]+(prior or req.history)+[{'role':'user','content':req.message}]
    try: answer=chat(messages)
    except Exception as exc: raise HTTPException(status_code=503,detail=str(exc)) from exc
    save_message(req.session_id,'user',req.message); save_message(req.session_id,'assistant',answer)
